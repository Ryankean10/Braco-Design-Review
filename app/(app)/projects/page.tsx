...
import { getStageColour, getStageOrder } from '@/lib/stageDefaults'

export const dynamic = 'force-dynamic'

export default async function ProjectsPage() {
  const { supabase, user, role, effectiveCompanyId, company } = await getCompanyContext()
  ...
  const [{ data: projects }, { data: stages }] = await Promise.all([
    projectQuery,
    supabase.from('project_stages').select('project_id, stage, status, signed_off_at'),
  ])

  // Determine which projects are fully complete (final stage signed off)
  const finalStageName = stageOrder[stageOrder.length - 1]
  const completedProjectIds = new Set<string>()
  for (const s of stages ?? []) {
    if (s.stage === finalStageName && s.signed_off_at) {
      completedProjectIds.add(s.project_id)
    }
  }

  const activeProjects = (projects ?? []).filter(p => !completedProjectIds.has(p.id))
  const completedProjects = (projects ?? []).filter(p => completedProjectIds.has(p.id))
  ...
  for (const pid of Object.keys(activeMap)) {
    activeMap[pid].sort((a, b) => stageOrder.indexOf(a) - stageOrder.indexOf(b))
  }
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      {completedProjects.length > 0 && (
        <div>
          ...
          <div className="rounded-xl border opacity-70" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
            {colHeader}
            <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
